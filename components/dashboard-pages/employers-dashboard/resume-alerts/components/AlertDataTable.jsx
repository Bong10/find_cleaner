...
const AlertDataTable = () => {
  ...
  useEffect(() => {
    ...
    // You can extend mapping for 'job'|'message' via server filters if supported
  ...
  const handleDeleteFromList = async (notificationId, notificationTitle) => {
    ...
    if (shouldDelete) {
      // TODO: Call backend API to delete, then refresh list
      dispatch(fetchNotificationsAction({}));
    }
  };

  const handleDeleteFromDetails = (notificationId) => {
    // After deletion in details, refresh the list
    dispatch(fetchNotificationsAction({}));
  };
  ...
  const handleView = (n) => {
    ...
    if ((n?.type === 'Booking' || n?.category === 'booking')) {
      ...
      router.push(`/employers-dashboard/bookings${qs}`);
    } else if (t.type === 'job' && t.id) {
      router.push(`/job-single-v1/${t.id}`);
    } else if (t.type === 'chat' && t.id) {
      router.push(`/employers-dashboard/messages?chat=${t.id}`);
    } else {
