...
const ContentField = () => {
  ...
  const cleanerName = cleaner?.user?.name || cleaner?.user?.email?.split('@')[0] || "You";
  const employerName = employer?.user?.name || employer?.user?.company_name || employer?.user?.email?.split('@')[0] || "Employer";
