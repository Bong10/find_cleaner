...
const ContactList = () => {
  ...
  // Local messages cache for instant preview updates
  ...
  const getParticipant = (chat) => {
    // For cleaner view, the other person is the employer
    return chat?.employer || null;
  };
  ...
  const getLastMessagePreview = (chat) => {
    ...
    const isMine = senderSide === 'cleaner'; // 'Mine' is the cleaner in this context
  ...
  // Derive unread like employer: prefer local is_read, else server fields; active chat always 0
  const deriveUnreadForChat = (chat) => {
    ...
    if (hasIsReadField) {
      ...
      for (const m of local) {
        ...
        // For cleaner view, unread are employer messages
        if (side === 'employer') count += 1;
      }
      return count;
    }
    const candidates = [
        ...
        <div className="contacts-list">
          {list.length === 0 ? (
            <div className="empty-state">
              ...
              <small className="text-muted">Your chats with employers will appear here.</small>
            </div>
          ) : (
            list.map((chat) => {
              ...
              const name = participant?.company_name || participant?.user?.name || "TidyLinker Employer";
              const avatar =
                participant?.user?.profile_picture || "/images/resource/employer-1.png";
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    // Optimistic clear for snappy UX; ContentField will call server
